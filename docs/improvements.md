...
- [x] T2-10. 캐싱 레이어 (Caffeine → Redis) — 해결일: 2026-05-28 (Caffeine 단독, Redis 전환은 멀티 인스턴스 시점으로 보류)
  - 해결 PR/커밋: feat: T2-10 Caffeine 캐시 (categoryNames + categoryAliasMap + ingredientGroupsVisible)
  - 비고: **(1)** 의존성 — `spring-boot-starter-cache` + `caffeine` (TS-1 패턴 따라 starter 필수). **(2)** `application.yaml`에 `spring.cache.type=caffeine` + `cache-names: [categoryNames, categoryAliasMap, ingredientGroupsVisible]` + `caffeine.spec=maximumSize=2000,expireAfterWrite=30m` (단일 통합 TTL — per-cache 분리는 의도적 비포함). **(3)** `CacheConfig`(`@EnableCaching`) 신규. **(4)** 캐싱 대상 3개:
    - `CategoryService.findAllNames` `@Cacheable("categoryNames")` — datalist 렌더링 핫패스
    - `CategoryAliasService.resolve` `@Cacheable("categoryAliasMap", key="#input", condition="#input != null and !#input.isBlank()")` — 원가 계산 행마다 호출 최핫패스. **condition 사용 이유는 TS-14 참조** (unless로는 null key 차단 불가)
    - `IngredientService.findAllVisible` `@Cacheable("ingredientGroupsVisible")` — recipe 폼 GET 4곳에서 호출
    **(5)** 무효화 — `ensureExists` / `CategoryAliasService.add·delete` / `IngredientService.updateCategory·fetchAndUpsert·delete`에 `@CacheEvict(allEntries=true)`. per-key evict는 input 표준화(trim/lowercase) 선행이 필요해 보수적으로 allEntries. **(6)** 테스트 격리 — `application-test.yaml`에 `spring.cache.type=none` (기존 124 테스트 영향 0). 캐시 동작 검증은 `CacheBehaviorTest`(`@SpringBootTest` + `properties` override + `@MockitoBean` repository)에서 8 케이스(스모크 1 + categoryNames 2 + alias 3 + ingredientVisible 2). **테스트**: 총 134(이전 126 → +8). **의도적 비포함**: 페이징 메서드 캐싱(page/size/q 가변성 ↑, 무효화 비용 > 이득), `viewByCategory`(T2-8 DB 캐시 + async refetch로 충분), per-cache TTL 분리(30분 통일이 어색하지 않음), Redis 전환(멀티 인스턴스 시점), 캐시 통계/메트릭(T2-13 Actuator와 같이), per-key evict.
...
- [x] T3-19. 가격 이력 (IngredientPriceHistory) — 해결일: 2026-05-25 (변동 시 적재 + admin UI; 일반 사용자 공개는 수익화 단계 3 도입 시)
  - 해결 PR/커밋: feat: T3-19 가격 이력 (변동 시 적재 + admin UI)
  - 비고: **(1)** Flyway `V4__ingredient_price_history.sql` — `ingredient_price_history(id, ingredient_id FK CASCADE, naver_product_id snapshot, price, total_amount, unit, price_per_gram, recorded_at)` + 인덱스 `(ingredient_id, recorded_at DESC)`. 기존 ingredients 백필. **(2)** `IngredientPriceHistory` 엔티티 + Repository(`findByIngredientIdOrderByRecordedAtDesc` + 기간 조회). 정적 팩토리 `snapshotOf(Ingredient, recordedAt)`. **(3)** 정책 옵션 B (사용자 선택): 가격 변동(pricePerGram `BigDecimal.compareTo != 0`)일 때만 적재. 신규는 첫 fetch 시 무조건. `IngredientService.fetchAndUpsert`에 통합 — 기존 갱신 직전 pricePerGram 백업 → refreshFromNaver 후 비교. `pricePerGram` 비교라 price 같지만 totalAmount 변동 케이스(1kg→1.5kg)도 잡힘. **(4)** Admin UI `/admin/ingredients/{id}/history` — 최신순 표 + 이전 행과 비교한 단가 차이(+/-/최초). list.html에 "이력" 링크. **(5)** 노출은 admin만(옵션 X) — 일반 사용자 공개는 수익화 단계 3 도입 시 무료/유료 라인 결정 후. **테스트**: IngredientServiceTest +4(신규 무조건/변동 적재/동일 skip/totalAmount 변동 잡힘) = **총 123(이전 119 → +4)**. **수익화 활용**: monetization.md 단계 3(Freemium 차트 차별화 — 무료 1주 / 유료 전기간) + 단계 4(데이터 라이선싱 12개월 누적 후) 공통 선행. **미해결**: 일반 사용자 공개 UI, 차트 시각화(JS 없음 정책상 SVG 또는 별도 라이브러리), 기간 필터, 카테고리별 평균 추이, archival 정책(1년+ 누적 시).
